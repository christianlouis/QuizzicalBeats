...
| #31 Server Stability production hardening | Partial | Gunicorn entrypoint, ProxyFix/HTTPS handling, secure cookie defaults, configurable app-level security headers with HSTS on HTTPS, and safe gzip compression for text-like responses | Reverse proxy/SSL termination validation, static asset optimization, and live production smoke remain. |
